@extends('layouts.app')
@section('title', 'Edit Jadwal Ruangan - SIM Sarpras')
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.jadwal_ruangan.update', $jadwalRuangan->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('ruangan_id') is-invalid @enderror" id="ruangan_id" name="ruangan_id" required>
                    ...
                    @foreach($ruangans as $r)
                        <option value="{{ $r->id }}" {{ old('ruangan_id', $jadwalRuangan->ruangan_id) == $r->id ? 'selected' : '' }}>{{ $r->nama_ruangan }} (Kapasitas: {{ $r->kapasitas }}) - {{ $r->lantai->gedung->nama }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('kegiatan') is-invalid @enderror" id="kegiatan" name="kegiatan" value="{{ old('kegiatan', $jadwalRuangan->kegiatan) }}" required>
                @error('kegiatan') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            <div class="row">
                <div class="col-md-4 mb-3">
                    <label for="tanggal" class="form-label fw-bold">Tanggal</label>
                    <input type="date" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal" name="tanggal" value="{{ old('tanggal', $jadwalRuangan->tanggal) }}" required>
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" rows="3">{{ old('keterangan', $jadwalRuangan->keterangan) }}</textarea>

@extends('layouts.app')
@section('title', 'Edit Lantai - SIM Sarpras')
@section('page_title', 'Edit Lantai')
@section('content')
<div class="card card-stat bg-white mb-4">
    <div class="card-body">
        <h5 class="card-title fw-bold text-success mb-4">Form Edit Lantai</h5>
        <form action="{{ route('admin.lantai.update', $lantai->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select class="form-select @error('gedung_id') is-invalid @enderror" id="gedung_id" name="gedung_id" required>
                    ...
                    @foreach($gedungs as $g)
                        <option value="{{ $g->id }}" {{ old('gedung_id', $lantai->gedung_id) == $g->id ? 'selected' : '' }}>{{ $g->nama }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('nomor_lantai') is-invalid @enderror" id="nomor_lantai" name="nomor_lantai" value="{{ old('nomor_lantai', $lantai->nomor_lantai) }}" required>
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" rows="3">{{ old('keterangan', $lantai->keterangan) }}</textarea>

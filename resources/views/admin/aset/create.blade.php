@extends('layouts.app')
@section('title', 'Tambah Aset - SIM Sarpras')
@section('page_title', 'Tambah Aset')
@section('content')
<div class="card card-stat bg-white mb-4">
    <div class="card-body">
        <h5 class="card-title fw-bold text-success mb-4">Form Tambah Aset</h5>
        <form action="{{ route('admin.aset.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('kode_aset') is-invalid @enderror" id="kode_aset" name="kode_aset" value="{{ old('kode_aset') }}" required>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('ruangan_id') is-invalid @enderror" id="ruangan_id" name="ruangan_id">
                        ...
                        @foreach($ruangans as $r)
                            <option value="{{ $r->id }}" {{ old('ruangan_id') == $r->id ? 'selected' : '' }}>{{ $r->nama_ruangan }} ({{ $r->kode_ruangan }})</option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="col-md-3 mb-3">
                    ...
                    <select class="form-select @error('tipe_aset') is-invalid @enderror" id="tipe_aset" name="tipe_aset" required>
                        <option value="individual" {{ old('tipe_aset') == 'individual' ? 'selected' : '' }}>Individual</option>
                        <option value="jumlah" {{ old('tipe_aset') == 'jumlah' ? 'selected' : '' }}>Jumlah (Quantity)</option>
                    </select>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select class="form-select @error('dapat_dipinjam') is-invalid @enderror" id="dapat_dipinjam" name="dapat_dipinjam" required>
                        <option value="1" {{ old('dapat_dipinjam', '1') == '1' ? 'selected' : '' }}>Ya</option>
                        <option value="0" {{ old('dapat_dipinjam') == '0' ? 'selected' : '' }}>Tidak</option>
                    </select>
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <label for="foto" class="form-label fw-bold">Foto Aset</label>

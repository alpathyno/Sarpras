@extends('layouts.app')
@section('title', 'Edit Ruangan - SIM Sarpras')
@section('page_title', 'Edit Ruangan')
@section('content')
<div class="card card-stat bg-white mb-4">
    <div class="card-body">
        <h5 class="card-title fw-bold text-success mb-4">Form Edit Ruangan</h5>
        <form action="{{ route('admin.ruangan.update', $ruangan->id) }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('lantai_id') is-invalid @enderror" id="lantai_id" name="lantai_id" required>
                        ...
                        @foreach($lantais as $l)
                            <option value="{{ $l->id }}" {{ old('lantai_id', $ruangan->lantai_id) == $l->id ? 'selected' : '' }}>{{ $l->gedung->nama }} - Lantai {{ $l->nomor_lantai }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('kode_ruangan') is-invalid @enderror" id="kode_ruangan" name="kode_ruangan" value="{{ old('kode_ruangan', $ruangan->kode_ruangan) }}" required>
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('jenis') is-invalid @enderror" id="jenis" name="jenis" value="{{ old('jenis', $ruangan->jenis) }}" required>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" class="form-control @error('kapasitas') is-invalid @enderror" id="kapasitas" name="kapasitas" value="{{ old('kapasitas', $ruangan->kapasitas) }}" required min="0">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('dapat_dipinjam') is-invalid @enderror" id="dapat_dipinjam" name="dapat_dipinjam" required>
                        <option value="1" {{ old('dapat_dipinjam', $ruangan->dapat_dipinjam) == '1' ? 'selected' : '' }}>Ya</option>
                        <option value="0" {{ old('dapat_dipinjam', $ruangan->dapat_dipinjam) == '0' ? 'selected' : '' }}>Tidak</option>
                    </select>
                ...
                <div class="col-md-4 mb-4">
                    ...
                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                        ...
                        <option value="Dalam Perbaikan" {{ old('status', $ruangan->status) == 'Dalam Perbaikan' ? 'selected' : '' }}>Dalam Perbaikan</option>
                        <option value="Tidak Digunakan" {{ old('status', $ruangan->status) == 'Tidak Digunakan' ? 'selected' : '' }}>Tidak Digunakan</option>
                    </select>
